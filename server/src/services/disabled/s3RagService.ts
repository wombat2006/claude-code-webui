...
export class S3RagService {
  ...
  async storeDocument(document: Omit<RagDocument, 'id'>): Promise<RagDocument> {
    try {
      ...
      logger.error('Failed to store RAG document:', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async getDocument(documentId: string): Promise<RagDocument | null> {
    try {
      ...
      logger.error(`Failed to get RAG document ${documentId}:`, error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async searchDocuments(query: RagSearchQuery): Promise<RagSearchResult[]> {
    try {
      ...
      logger.error('Failed to search RAG documents:', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
  ...
  async deleteDocument(documentId: string): Promise<boolean> {
    try {
      ...
      logger.error(`Failed to delete RAG document ${documentId}:`, error instanceof Error ? error : new Error(String(error)));
      return false;
    }
  }
  ...
  async cleanupOldDocuments(daysOld: number = 30): Promise<number> {
    try {
      ...
      logger.error('Failed to cleanup old RAG documents:', error instanceof Error ? error : new Error(String(error)));
      return 0;
    }
  }
  ...
  async getStatistics(): Promise<{
    ...
    try {
      ...
      logger.error('Failed to get RAG service statistics:', error instanceof Error ? error : new Error(String(error)));
      throw error;
    }
  }
}
